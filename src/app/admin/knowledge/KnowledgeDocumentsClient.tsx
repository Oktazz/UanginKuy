...
export default function KnowledgeDocumentsClient({
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setNotice(validationError ? { type: "error", message: validationError } : null);
  };
  ...
  const handleDelete = async (document: KnowledgeDocumentListItem) => {
    if (!window.confirm(`Hapus “${document.title}” dari knowledge AI?`)) return;

    setDeletingId(document.id);
    setNotice(null);
    try {
      const response = await fetch(`/api/admin/knowledge/documents/${document.id}`, {
        method: "DELETE",
      });
      if (!response.ok) {
      ...
      setNotice({ type: "success", message: "Dokumen dan embedding berhasil dihapus." });
      router.refresh();
    } catch (error) {
      setNotice({
        type: "error",
        message: error instanceof Error ? error.message : "Dokumen gagal dihapus.",
      });
    } finally {
      setDeletingId(null);
    }
  ...
  const handlePreview = async (document: KnowledgeDocumentListItem) => {
    ...
    try {
      ...
      setPreviewError(
        error instanceof Error ? error.message : "Preview dokumen gagal dimuat.",
      );
    } finally {
    ...
    <div className="space-y-8">
      <section className="rounded-3xl border border-gray-100 bg-surface p-6 shadow-sm lg:p-8">
        <form ref={formRef} onSubmit={handleUpload} className="space-y-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)_auto] lg:items-end">
            <div>
              <label htmlFor="knowledge-title" className="mb-2 block text-sm font-bold text-gray-700">
                Judul knowledge <span className="font-medium text-gray-400">(opsional)</span>
              </label>
              <input
                id="knowledge-title"
                name="title"
            ...
            <div>
              <label htmlFor="knowledge-file" className="mb-2 block text-sm font-bold text-gray-700">
                Dokumen
              </label>
            ...
            >
              {isUploading ? <Loader2 className="animate-spin" size={19} /> : <UploadCloud size={19} />}
              <span className="ml-2">{isUploading ? "Memproses..." : "Upload & Embed"}</span>
            </button>
          </div>
          <p className="text-sm font-medium text-gray-500">
            Maksimal 6 MB. PDF hasil scan tanpa teks belum didukung; gunakan PDF berbasis teks atau DOCX.
          </p>
        </form>
        ...
        {notice && (
          ...
          >
            {notice.type === "success" && <CheckCircle2 size={18} className="mt-0.5 shrink-0" />}
      ...
      <section className="overflow-hidden rounded-3xl border border-gray-100 bg-surface shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5 lg:px-8">
          <div>
            <h2 className="text-lg font-extrabold text-gray-900">Dokumen terindeks</h2>
            <p className="mt-1 text-sm font-medium text-gray-500">
              Hapus dokumen bila isinya tidak lagi boleh menjadi referensi UanginBot.
            </p>
          </div>
        ...
        {documents.length === 0 ? (
          <div className="px-6 py-16 text-center">
            <FileText size={40} className="mx-auto text-gray-300" aria-hidden="true" />
            <p className="mt-4 font-bold text-gray-700">Belum ada knowledge dari dokumen</p>
            <p className="mt-1 text-sm font-medium text-gray-500">Unggah panduan, SOP, atau FAQ pertama Anda.</p>
          </div>
        ) : (
                  ...
                  <tr key={document.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex rounded-full bg-success/10 px-3 py-1 text-xs font-extrabold text-success">
                        {document.status === "active" ? "Aktif" : document.status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      >
                        {previewingId === document.id ? <Loader2 className="animate-spin" size={18} /> : <Eye size={18} />}
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        {deletingId === document.id ? <Loader2 className="animate-spin" size={18} /> : <Trash2 size={18} />}
                      </button>
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-surface shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-gray-100 px-6 py-4">
              <div className="min-w-0">
                <h2 id="knowledge-preview-title" className="truncate text-lg font-extrabold text-gray-900">
                  {preview?.title ?? "Preview dokumen"}
                </h2>
                {preview && <p className="mt-1 truncate text-sm font-medium text-gray-500">{preview.originalName}</p>}
              </div>
              <button
                type="button"
                aria-label="Tutup preview"
                onClick={() => { setPreview(null); setPreviewError(null); }}
                className="rounded-xl px-3 py-2 text-sm font-bold text-gray-500 hover:bg-gray-100 hover:text-gray-900"
              >
                Tutup
              </button>
            </div>
            {previewError ? (
              <p className="p-8 text-sm font-semibold text-error">{previewError}</p>
